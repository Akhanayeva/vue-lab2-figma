<template>
  <div class="cards">
    ...
    <div class="cards-section">
      <div v-for="person in paginatedPersons" :key="person.id" class="card">
    ...
    <div class="navigation-buttons">
      ...
      <button @click="nextCards" :disabled="currentPage >= totalPages">Next</button>
    </div>
        ...
        export default {
          ...
          },
          {
            ...
            topic: 'Sport'
              ...
              currentPage: 1,
              selectedSortMethod: '' // Новая переменная для метода сортировки
            };
          },
          computed: {
            ...
            filteredPersons() {
              if (!this.selectedTopic) {
                return this.persons; // Если тема не выбрана, возвращаем все карточки
              }
              return this.persons.filter(person => person.topic === this.selectedTopic);
            },
            // Сортировка отфильтрованных карточек
            sortedPersons() {
    let sortedArray = [...this.filteredPersons]; // Create a copy of the array to avoid mutating the original one.
    
    if (this.selectedSortMethod === 'date') {
      return sortedArray.sort((a, b) => new Date(a.pubDate) - new Date(b.pubDate));
    } else if (this.selectedSortMethod === 'rating') {
      return sortedArray.sort((a, b) => b.rating - a.rating);
    }
    
    return sortedArray; // If no sort method is selected, return the copied array.
  },
            // Общие количество страниц
            totalPages() {
              return Math.ceil(this.sortedPersons.length / 4);
            },
            // Пагинация: получение карточек для текущей страницы
            paginatedPersons() {
              const startIndex = (this.currentPage - 1) * 4;
              return this.sortedPersons.slice(startIndex, startIndex + 4);
            }
          },
          methods: {
            increaseRating(id) {
              ...
              if (person) {
                person.rating = Math.min(person.rating + 0.5, 5.0);
              }
            },
            nextCards() {
          ...
          watch: {
            // Сбрасывать текущую страницу при изменении выбранной темы
            selectedTopic() {
              this.currentPage = 1;
            }
          }
    ...
    gap: 25px; /* Set a consistent gap between rows and columns */
    margin-top: 245px;
